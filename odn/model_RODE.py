'''
Models tested and used for RODENet.
Brent Griffin, griffb@umich.edu, 190424
'''
...
# ResOpt
def pass_arg(**kwargs):
	model = ResOptNet(CustomBlock, **kwargs)
	return model

class ResOptNet(nn.Module):
	def __init__(self, block, layr=[1], kern=[1], fcs=[512], n1=5, n2=5, 
			n2_in=True, n2_fc=True, avg_pool=7, par_net=False, par_n=1,
			batch_nrm=True, n2_mid=True, mid_rep=1, strd=[1], conv_kern=14,
			max_pool=True, fc_drop=0):
		super(ResOptNet, self).__init__()
		# Initial config.
...
def rode_res_224_nb(**kwargs):
	model = RODEResNet(NoBatchBlock, [2, 2, 2, 2, 2, 2], 
			[1, 1, 2, 2, 4, 4], [16, 8], **kwargs)
	return model

def rode_res_224(**kwargs):
	model = RODEResNet(BasicBlock, [2, 2, 2, 2, 2, 2], 
			[1, 1, 2, 2, 4, 4], [16, 8, 4, 2], **kwargs)
	return model

def rode_res_480_ro(**kwargs):
	# Include residual optics input.
	model = RODEResNet(BasicBlock, [2, 2, 2, 2, 2, 2, 2], 
			[64, 64, 128, 128, 256, 256, 256], [1536, 768, 384, 192], **kwargs)
	return model

def rode_res_480(**kwargs):
	model = RODEResNet(BasicBlock, [2, 2, 2, 2, 2, 2, 2], 
			[64, 64, 128, 128, 256, 256, 256], [1536, 768, 384, 192], **kwargs)
	#model = RODEResNet(BasicBlock, [2, 2, 2, 2, 2, 2, 2], 
			#[1, 1, 2, 2, 4, 4, 4], [24, 12], **kwargs) # 4 x 6
	#model = RODEResNet(BasicBlock, [2, 2, 2, 2, 2, 2, 2], 
			#[8, 8, 16, 16, 32, 32, 32], [192, 96], **kwargs)
	return model

def roderesnet34(**kwargs):
	#model = RODEResNet(BasicBlock, [3, 4, 6, 3], [16, 32, 64, 128], **kwargs)
	model = RODEResNet(BasicBlock, [3, 4, 6, 3], [32, 64, 128, 256], **kwargs)
	return model

def roderesnet18(**kwargs):
	#model = RODEResNet(BasicBlock, [2, 2, 2, 2], [32, 64, 128, 256], **kwargs)
	#model = RODEResNet(BasicBlock, [2, 2, 2, 2], [16, 32, 64, 128], **kwargs)
	model = RODEResNet(BasicBlock, [2, 2, 2, 2, 2], [32, 64, 128, 256, 512], 
			**kwargs)
	return model

class RODEResNet(nn.Module):
	def __init__(self, block, layers, kernels, fcs=[512], norm=True, n=5):
		self.inplanes = kernels[0]
		super(RODEResNet, self).__init__()
		self.conv1 = nn.Conv2d(n, self.inplanes, kernel_size=7, stride=2, 
		...
		self.layer5 = self._make_layer(block, kernels[4], layers[4], stride=2)
		#self.avgpool = nn.AvgPool2d((8,10), stride=1)
		self.layer6 = self._make_layer(block, kernels[5], layers[5], stride=2)
		self.layer7 = self._make_layer(block, kernels[6], layers[6], stride=2)
		self.fc0 = nn.Linear(fcs[0] + (n-1), fcs[1])
		self.fc1 = nn.Linear(fcs[1], fcs[2])
		self.fc2 = nn.Linear(fcs[2], fcs[3])
		self.fc = nn.Linear(fcs[-1] * block.expansion, 1)

	def _make_layer(self, block, planes, blocks, stride=1, norm=True):
		downsample = None
		if stride !=1 or self.inplanes != planes * block.expansion:
			if norm:
				downsample = nn.Sequential(
	...
	def forward(self, x, y):
		x = self.conv1(x) # 480x640 (224x224)
		x = self.bn1(x)
		x = self.relu(x)
		x = self.maxpool(x) # 240x320 (112x112)

		x = self.layer1(x) # 120x160 (56x56)
		# Concatenate here for Residual Optics (see notes pg. 16 on 190710)
		x = self.layer2(x) # 60x80 (28x28)
		x = self.layer3(x) # 30x40 (14x14)
		x = self.layer4(x) # 15x20 (7x7)
		x = self.layer5(x) # 8x10 (4x4)
		x = self.layer6(x) # 4x5 (2x2)
		x = self.layer7(x) # 2x3
...
class NoBatchBlock(nn.Module):
	expansion = 1
	def __init__(self, inplanes, planes, stride=1, downsample=None):
		super(NoBatchBlock, self).__init__()
		self.conv1 = conv3x3(inplanes, planes, stride)
		self.relu = nn.ReLU(inplace=True)
		self.conv2 = conv3x3(planes, planes)
		self.downsample = downsample
...
class ResNet(nn.Module):
	def __init__(self, block, layers, num_classes=1):
		...
		self.conv1 = nn.Conv2d(2, self.inplanes, kernel_size=7, stride=2, 
				padding=3, bias=False)
	...
	def forward(self, x):
		x = self.conv1(x) # 224x224 (RODE: 480x640x2)
		x = self.bn1(x)
		x = self.relu(x)
		x = self.maxpool(x) # 112x112 (RODE: 240x320)

		x = self.layer1(x) # 56x56 (RODE: 120x160)
		x = self.layer2(x) # 28x28 (RODE: 60x80)
		x = self.layer3(x) # 14x14 (RODE: 30x40)
		x = self.layer4(x) # 7x7   (RODE: 15x20)
...
class LeNet(nn.Module):
	def __init__(self):
		...
		self.conv1 = nn.Conv2d(2, 6, 5) # 2 channel for two binary masks.
		#self.conv1 = nn.Conv2d(1, 6, 5)
		self.pool = nn.MaxPool2d(2, 2) # No pooling in future!
		self.conv2 = nn.Conv2d(6, 16, 5)
		self.fc1 = nn.Linear(16 * 117 * 157, 120)
		#self.fc1 = nn.Linear(16 * 5 * 5, 120)
		self.fc2 = nn.Linear(120, 84)
		self.fc3 = nn.Linear(84, 1) # Need output to be 1 (RO depth)
		#self.fc3 = nn.Linear(84, 10)

	def forward(self, x):
		x = self.pool(F.relu(self.conv1(x)))
		x = self.pool(F.relu(self.conv2(x)))
		x = x.view(-1, 16 * 117 * 157) # Account for larger input size.
		#x = x.view(-1, 16 * 5 * 5)
		x = F.relu(self.fc1(x))
		x = F.relu(self.fc2(x))
		x = self.fc3(x)
